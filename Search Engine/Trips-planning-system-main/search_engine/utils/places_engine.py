...
class PlacesSearchEngine:
    def __init__(self):
        self.otm_url = 'http://api.opentripmap.com/0.1/en/places'
        self.otm_api_key = '5ae2e3f221c38a28845f05b61f6983d09ef19a35837145ac8fbff9e7'
    ...
    def search_places_by_coords(self, latitude, longitude, kind='', rate=''):
        if len(kind):
            kind = '&kinds={}'.format(kind)
        if len(rate):
    ...
    def get_top_rated_places(self, latitude, longitude, places_count, kind='', rate=''):
        res_plcs = self.search_places_by_coords(latitude, longitude, kind, rate)['features']
    ...
    def clean_results(self, data):
        ...
        for item in data:
            res.append({
                ...
                'kinds': _clean_kinds(item['properties']['kinds']),
